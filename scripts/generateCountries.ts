...
const generateCountries = (): Countries => {
  ...
  const countries = Object.keys(data).reduce((acc, key) => {
    const datum = data[key];
    const countryAlpha2 = datum.countryId?.slice(-2);
    const countryAlpha3 =
      ALPHA2_TO_ALPHA3[countryAlpha2 as keyof typeof ALPHA2_TO_ALPHA3];
    if (countryAlpha3) {
      acc[ALPHA2_TO_ALPHA3[countryAlpha2 as keyof typeof ALPHA2_TO_ALPHA3]] =
        datum.country;
    }
    return acc;
  }, {} as Countries);
